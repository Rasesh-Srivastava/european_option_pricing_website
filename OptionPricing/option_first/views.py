...
def d1(S,K,T,r,sigma):
    return 0.0 if T==0 else (log(S/K)+(r+(sigma**2)/2.0)*T)/(sigma*sqrt(T))
def d2(S,K,T,r,sigma):
    return 0.0 if T==0 else d1(S,K,T,r,sigma)-sigma*sqrt(T)
...
def calc_p(r,T,u,d,n=2.0):
    if(T!=0.0):
        t = float(T)
        R = float(r)
        p = float(0.0 if u==0 else (math.exp(R*(t/n)) - d)/(u-d))
        return p
    else:
        return 0.0
...
def home(request):
    ...
    # print(type(S))
    # print(T)
    return render(request, 'option_first/home.html',{'title': 'Rasesh','S': S,'T': T,'K': K,'r': r*100,'sigma': sigma,'u': u,'d': d,'p': p,'Cuu':Cuu,'Cdd':Cdd,'Cud':Cud,'C':C,'P':P,'bs_C':bs_C,'bs_P':bs_P,'n_C':n_C,'n_P':n_P,'n':n,'u_n':u_n,'d_n':d_n,'a':a,'p_n':p_n,'p_dash':p_dash})
def black(request):
    return render(request, 'option_first/black.html')
